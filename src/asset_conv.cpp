...
/// square. Does not matter if it does not fit in the resulting image, it will
/// //// simply be cropped.
struct TaskDef {
  std::string fname_in;
...
const int DEF_NUM_THREADS = 1; // Default value, changed by argv.
...
public:
  ...
  void operator()(size_t width, size_t height, size_t BPP,
                  const unsigned char *image_data, size_t stride) {
    stbi_write_func *fun = PNGWriter::rawCallback;
    int r = stbi_write_png_to_func(fun, this, width, height, BPP,
                                   &image_data[0], stride);
  ...
  PNGDataPtr getData() { return png_data_; }
};
...
///
/// TODO (done): Process assets in a thread pool.
...
public:
  ...
  ~Processor() {
    should_run_ = false; // is atomic, no need to make it fast, happens once
...
private:
  ...
  static bool parse_task(const std::string &line_org, TaskDef &def) {
    ...
    if (tokens.size() < 3) {
      std::cerr << "Error: Wrong line format: " << line_org
                << " (size: " << line_org.size() << ")." << std::endl;
      return false;
    }
